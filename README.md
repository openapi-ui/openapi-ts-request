...
根据 [Swagger2/OpenAPI3](https://swagger.io/blog/news/whats-new-in-openapi-3-0/) 文档生成 TS 类型, 客户端请求函数,
模拟请求响应服务, 枚举, 类型字段翻译, JSON Schemas
...
> 配置文件还支持 ***.openapi-ts-request.ts***, ***openapi-ts-request.config.cjs***
> 等格式，参考 [cosmiconfig](https://github.com/cosmiconfig/cosmiconfig?tab=readme-ov-file#cosmiconfig)
...
| templatesFolder        | 否  | string                      | './templates'                | 模版路径,可以在项目自定义                                        | 
| allowedTags            | 否  | string[]                    | -                            | 只解析归属于 tags 集合的api 和 schema                          | 
| excludeTags            | 否  | string[]                    | -                            | 排除tags                                               | 
| allowedPaths           | 否  | string[]                    | -                            | 只解析归属于 paths 集合的api                                  | 
| excludePaths           | 否  | string[]                    | -                            | 排除paths                                              | 
...
- 默认生成 [components.schemas](https://spec.openapis.org/oas/latest.html#components-object) 下面的 JSON
  Schemas，[paths](https://spec.openapis.org/oas/latest.html#paths-object) 对应的 JSON Schemas 目前需自行解析
...
目前使用 [mockjs](http://mockjs.com) 生成 mock 数据，mocks
文件启动需要借助 [@umijs/server](https://umijs.org/docs/guides/mock)，后面会寻找其他方案以达到更好的 mock 体验
